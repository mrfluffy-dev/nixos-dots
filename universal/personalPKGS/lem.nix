{ sbcl, cl-charms, jsonrpc, queues, openssl, micros, makeWrapper
, fetchFromGitHub, writeText, lib, symlinkJoin, lem }:
...
sbcl.buildASDFSystem rec {
  ...
  passthru = {
    withPackages =
      import ./wrapper.nix { inherit makeWrapper sbcl lib lem symlinkJoin; };
  };
}
