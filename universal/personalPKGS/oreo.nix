...
lib.checkListOfEnum "${pname}: color variants"
  ...
  stdenv.mkDerivation
  {
    ...
    version = "final";
    src = fetchurl {
      url = "https://git.mrfluffy.xyz/mrfluffy/oreo-cursors/archive/final.tar.gz";
      hash = "sha256-Ke4+HyzfZRxNB9ER8UbozFu1+WqRK3PkiwK/lPyr8lc=";
    };
    nativeBuildInputs = [ ];
    ...
    installPhase = ''
      ...
      if [[ " ${builtins.toString colors} " =~ " all " ]]; then
        ...
        for color in ${builtins.toString colors}; do
            cp -r cursors/$color $out/share/icons/
        done
      fi
